...
**Remediation:** For HTTP 403 responses with tenant enforcement enabled, verify the gateway-injected tenant dimensions before changing the projected service-account token or subject allowlist. If they are correct, check the token and allowlist. Once credentials are fixed, the HyperFleet operator will reconcile the affected resources; no redelivery is needed.
...
2. Republish the event to the broker topic. The event payload must conform to the [async API contract](https://github.com/openshift-hyperfleet/architecture/blob/main/hyperfleet/components/broker/asyncapi.yaml).
