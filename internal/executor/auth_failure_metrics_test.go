...
func TestCreateHandler_PostActionAPIAuthFailuresAreAcknowledgedAndRecorded(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = handler(t.Context(), authFailureEvent(t, "cluster-auth-failure"))
...
func TestCreateHandler_PostActionNonAuthAPIFailureDoesNotEmitAuthMetric(t *testing.T) {
	...
	handler := AlwaysAck(WithMetrics(exec.CreateHandler(), recorder))
	err = handler(t.Context(), authFailureEvent(t, "cluster-non-auth-failure"))
...
func TestCreateHandler_OptionalAPIParameterAuthFailureIsLoggedAndRecorded(t *testing.T) {
	...
	result, err := WithMetrics(exec.CreateHandler(), recorder)(
		t.Context(), authFailureEvent(t, "cluster-optional-auth-failure"))
	require.NoError(t, err)
...
func TestExecutor_PostActionAuthFailureLogsAreContextualAndRedacted(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := exec.Execute(t.Context(), map[string]interface{}{
				"id":   "cluster-auth-401",
				"kind": "Cluster",
			})
			...
			executionMessage := result.ExecutionContext.Adapter.ExecutionError.Message
			require.Len(t, result.PostActionResults, 1)
			postActionResponse := string(result.PostActionResults[0].APIResponse)
			if tt.expectResponseBody {
				assert.Contains(t, captured, tt.sentinel,
					"non-auth response details must remain available in executor logs")
				assert.Contains(t, executionMessage, tt.sentinel,
					"non-auth response details must remain available to status conditions")
				assert.Contains(t, postActionResponse, tt.sentinel,
					"non-auth response details must remain available in post-action results")
			} else {
				assert.NotContains(t, captured, tt.sentinel,
					"auth response bodies must not be written to executor logs")
				assert.NotContains(t, executionMessage, tt.sentinel,
					"auth response bodies must not reach status conditions")
				assert.NotContains(t, postActionResponse, tt.sentinel,
					"auth response bodies must not remain in post-action results")
			}
			if !tt.expectAuthLog {
...
func TestExecutor_APIAuthFailuresAreLoggedAcrossExecutionPhases(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			contextKey:   "precondition",
			contextValue: "fetch-cluster",
			config: &configloader.Config{
				Adapter: configloader.AdapterInfo{Name: "test-adapter", Version: "v0.1.0"},
				Params: []configloader.Parameter{
					{Name: "clusterID", Source: configloader.StringSource("event.id"), Required: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := WithMetrics(exec.CreateHandler(), recorder)(
				t.Context(), authFailureEvent(t, "cluster-auth-failure"))
			require.NoError(t, err)
			require.Equal(t, StatusFailed, result.Status)

			assert.Contains(t, logs.String(), "http_status=403")
			assert.Contains(t, logs.String(), "phase="+string(tt.phase))
			assert.Contains(t, logs.String(), tt.contextKey+"="+tt.contextValue)
			assert.Contains(t, logs.String(), "resource_id=cluster-auth-failure")
