...
func (re *ResourceExecutor) executeResource(ctx context.Context, resource config_loader.Resource, execCtx *ExecutionContext) (ResourceResult, error) {
	...
	re.log.Infof(ctx, "Resource[%s] is processing: operation=%s kind=%s name=%s reason=%s", resource.Name, strings.ToUpper(string(result.Operation)), gvk.Kind, manifest.GetName(), result.OperationReason)
	...
	if err != nil {
		...
		re.log.Errorf(ctx, "Resource[%s] processed: FAILED - operation=%s reason=%s kind=%s name=%s error=%v", resource.Name, strings.ToUpper(string(result.Operation)), result.OperationReason, gvk.Kind, manifest.GetName(), err)
		return result, NewExecutorError(PhaseResources, resource.Name,
			fmt.Sprintf("failed to %s resource", result.Operation), err)
	}
	re.log.Infof(ctx, "Resource[%s] processed: SUCCESS - operation=%s reason=%s kind=%s name=%s", resource.Name, strings.ToUpper(string(result.Operation)), result.OperationReason, gvk.Kind, manifest.GetName())
