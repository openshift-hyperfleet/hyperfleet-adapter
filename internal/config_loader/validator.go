...
func (v *Validator) ValidateStructure() error {
	...
	customValidators := []func() error{
		v.validateAPIVersionSupported,   // Check apiVersion is in supported list
		v.validateEnvParams,             // Check required env params have values
		v.validateDuplicateResourceNames, // Check for duplicate resource names
	}
...
// validateEnvParams checks that required env params have values set
func (v *Validator) validateEnvParams() error {
...
// validateDuplicateResourceNames checks for duplicate resource names
func (v *Validator) validateDuplicateResourceNames() error {
...
// =============================================================================
// SEMANTIC VALIDATION: Capture Fields
// =============================================================================
...
func (v *Validator) validateCaptureFields() {
	for i, precond := range v.config.Spec.Preconditions {
		for j, capture := range precond.Capture {
			...
			v.validateCaptureFieldExpression(capture, path)
		}
	}
}

func (v *Validator) validateCaptureFieldExpression(capture CaptureField, path string) {
	// Structural validation (name required, field/expression mutual exclusivity)
	// is handled by struct validator tags in types.go.
	// This validates CEL expression syntax.
	...
	for i, resource := range v.config.Spec.Resources {
		...
		if manifest, ok := resource.Manifest.(map[string]interface{}); ok {
			if ref, hasRef := manifest[FieldRef].(string); hasRef {
				...
				// Multiple refs: validated via ManifestItems
			} else {
...
// newValidator is a backward-compatible alias for NewValidator (for tests)
...
// (backward compatible with old Validator.Validate() interface)
func (v *Validator) Validate() error {
	return v.ValidateSemantic()
}

// validateFileReferences is a package-level helper for backward compatibility
...
// validateResources is a package-level helper for backward compatibility
