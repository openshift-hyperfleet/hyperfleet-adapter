...
func (v *Validator) validateConditionOperators() {
	...
	for i, precond := range v.config.Spec.Preconditions {
		for j, cond := range precond.Conditions {
			path := fmt.Sprintf("spec.preconditions[%d].conditions[%d]", i, j)
		...
		for i, action := range v.config.Spec.Post.PostActions {
			if action.When != nil {
				for j, cond := range action.When.Conditions {
					path := fmt.Sprintf("spec.post.postActions[%d].when.conditions[%d]", i, j)
					v.validateOperator(cond.Operator, path)
				}
			}
		}
...
func (v *Validator) validateTemplateVariables() {
	...
	for i, precond := range v.config.Spec.Preconditions {
		if precond.APICall != nil {
			v.validateTemplateString(precond.APICall.URL,
				fmt.Sprintf("spec.preconditions[%d].apiCall.url", i))
			v.validateTemplateString(precond.APICall.Body,
				fmt.Sprintf("spec.preconditions[%d].apiCall.body", i))
			for j, header := range precond.APICall.Headers {
				v.validateTemplateString(header.Value,
					fmt.Sprintf("spec.preconditions[%d].apiCall.headers[%d].value", i, j))
			}
		}
	}
	...
	for i, resource := range v.config.Spec.Resources {
		if manifest, ok := resource.Manifest.(map[string]interface{}); ok {
			v.validateTemplateMap(manifest, fmt.Sprintf("spec.resources[%d].manifest", i))
		}
		if resource.Discovery != nil {
			v.validateTemplateString(resource.Discovery.Namespace,
				fmt.Sprintf("spec.resources[%d].discovery.namespace", i))
			v.validateTemplateString(resource.Discovery.ByName,
				fmt.Sprintf("spec.resources[%d].discovery.byName", i))
			if resource.Discovery.BySelectors != nil {
				for k, val := range resource.Discovery.BySelectors.LabelSelector {
					v.validateTemplateString(val,
						fmt.Sprintf("spec.resources[%d].discovery.bySelectors.labelSelector[%s]", i, k))
				}
			}
		}
	}
	...
	if v.config.Spec.Post != nil {
		for i, action := range v.config.Spec.Post.PostActions {
			if action.APICall != nil {
				v.validateTemplateString(action.APICall.URL,
					fmt.Sprintf("spec.post.postActions[%d].apiCall.url", i))
				v.validateTemplateString(action.APICall.Body,
					fmt.Sprintf("spec.post.postActions[%d].apiCall.body", i))
				for j, header := range action.APICall.Headers {
					v.validateTemplateString(header.Value,
						fmt.Sprintf("spec.post.postActions[%d].apiCall.headers[%d].value", i, j))
				}
			}
		}
		...
		for i, param := range v.config.Spec.Post.Params {
			if param.Build != nil {
				if buildMap, ok := param.Build.(map[string]interface{}); ok {
					v.validateTemplateMap(buildMap, fmt.Sprintf("spec.post.params[%d].build", i))
				}
			}
		}
	}
	...
	for i, param := range v.config.Spec.Params {
		if param.Build != nil {
			if buildMap, ok := param.Build.(map[string]interface{}); ok {
				v.validateTemplateMap(buildMap, fmt.Sprintf("spec.params[%d].build", i))
			}
		}
	}
}
...
func (v *Validator) initCELEnv() error {
	...
	if !addedRoots["resources"] {
		options = append(options, cel.Variable("resources", cel.MapType(cel.StringType, cel.DynType)))
	}
...
func (v *Validator) validateCELExpressions() {
	...
	for i, precond := range v.config.Spec.Preconditions {
		if precond.Expression != "" {
			path := fmt.Sprintf("spec.preconditions[%d].expression", i)
	...
	if v.config.Spec.Post != nil {
		for i, action := range v.config.Spec.Post.PostActions {
			if action.When != nil && action.When.Expression != "" {
				path := fmt.Sprintf("spec.post.postActions[%d].when.expression", i)
		...
		for i, param := range v.config.Spec.Post.Params {
			if param.Build != nil {
				if buildMap, ok := param.Build.(map[string]interface{}); ok {
					v.validateBuildExpressions(buildMap, fmt.Sprintf("spec.post.params[%d].build", i))
				}
			}
		}
	}
...
func (v *Validator) validateK8sManifests() {
	for i, resource := range v.config.Spec.Resources {
		path := fmt.Sprintf("spec.resources[%d].manifest", i)

		// Validate inline or single-ref manifest
		if manifest, ok := resource.Manifest.(map[string]interface{}); ok {
			// Check for ref (external template reference)
			if ref, hasRef := manifest["ref"].(string); hasRef {
				if ref == "" {
					v.errors.Add(path+".ref", "manifest ref cannot be empty")
				}
			...
			} else if _, hasRefs := manifest["refs"]; hasRefs {
		...
		for j, item := range resource.ManifestItems {
			v.validateK8sManifest(item, fmt.Sprintf("spec.resources[%d].manifestItems[%d]", i, j))
		}
	}
}
...
func (v *Validator) validateK8sManifest(manifest map[string]interface{}, path string) {
	...
	if metadata, ok := manifest["metadata"].(map[string]interface{}); ok {
		if _, hasName := metadata["name"]; !hasName {
			v.errors.Add(path+".metadata", "missing required field \"name\"")
		}
	}
	...
	if apiVersion, ok := manifest["apiVersion"].(string); ok {
		if apiVersion == "" {
			v.errors.Add(path+".apiVersion", "apiVersion cannot be empty")
		}
	}
	...
	if kind, ok := manifest["kind"].(string); ok {
		if kind == "" {
			v.errors.Add(path+".kind", "kind cannot be empty")
		}
	}
}
