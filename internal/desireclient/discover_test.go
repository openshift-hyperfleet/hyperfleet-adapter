...
// putSyncedReadDesire creates a read desire and marks it Successful=True with
// the given content, the shape DiscoverResources expects for a converged mirror.
func putSyncedReadDesire(t *testing.T, ctx context.Context, store *memory.Store, name string, content []byte) {
	t.Helper()
	id := desire.Identity{
		ManagementCluster: testManagementCluster, Type: desire.TypeRead,
		Resource: testResource, Namespace: testNamespace, Name: name,
	}
...
func TestDiscoverResources_ReturnsSyncedResourceByName(t *testing.T) {
	...
	putSyncedReadDesire(t, ctx, store, testName, configMapManifest(1))
...
func TestDiscoverResources_LabelSelectorMatchesSubset(t *testing.T) {
	...
	putSyncedReadDesire(t, ctx, store, "labeled", labeledManifest)
	putSyncedReadDesire(t, ctx, store, "unlabeled", unlabeledManifest)
...
func TestDiscoverResources_SkipsFailedRead(t *testing.T) {
	ctx := context.Background()
	store := newMemoryStore()
	c := newTestClient(store)
	...
	list, err := c.DiscoverResources(ctx, testGVK(), &manifest.DiscoveryConfig{}, testTransportContext())
	require.NoError(t, err)
	assert.Empty(t, list.Items, "Successful=False must never surface content, even stale content")
}

func TestDiscoverResources_SkipsConfirmedAbsent(t *testing.T) {
	ctx := context.Background()
	store := newMemoryStore()
	c := newTestClient(store)
	...
	list, err := c.DiscoverResources(ctx, testGVK(), &manifest.DiscoveryConfig{}, testTransportContext())
	require.NoError(t, err)
	assert.Empty(t, list.Items, "empty KubeContent under Successful=True is confirmed-absent, not a match")
}
...
func TestDiscoverResources_SkipsUndecodableContentButKeepsOthers(t *testing.T) {
	...
	putSyncedReadDesire(t, ctx, store, "bad", []byte("not-json"))
	putSyncedReadDesire(t, ctx, store, "good", configMapManifest(1))
...
func TestDiscoverResources_MultipleMatches(t *testing.T) {
	...
	putSyncedReadDesire(t, ctx, store, "first", first)
	putSyncedReadDesire(t, ctx, store, "second", second)
