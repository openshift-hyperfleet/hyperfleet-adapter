...
func (m *MockK8sClient) CreateResource(
	ctx context.Context,
...
func (m *MockK8sClient) UpdateResource(
	ctx context.Context,
	obj *unstructured.Unstructured,
) (*unstructured.Unstructured, error) {
...
func (m *MockK8sClient) ApplyManifest(
	ctx context.Context,
...
// ApplyResource implements transportclient.TransportClient.ApplyResource
func (m *MockK8sClient) ApplyResource(
	ctx context.Context,
	manifestBytes []byte,
	...
	// Default behavior: parse and store (try JSON first, fall back to YAML)
	obj := &unstructured.Unstructured{}
	if err := json.Unmarshal(manifestBytes, &obj.Object); err != nil {
		jsonBytes, yamlErr := yaml.YAMLToJSON(manifestBytes)
		if yamlErr != nil {
			return nil, fmt.Errorf(
				"failed to parse manifest as JSON or YAML: %w", err)
		}
		if err := json.Unmarshal(jsonBytes, &obj.Object); err != nil {
			return nil, fmt.Errorf(
				"failed to parse manifest after YAML conversion: %w", err)
		}
	}
	key := obj.GetNamespace() + "/" + obj.GetName()
	m.Resources[key] = obj
...
func (m *MockK8sClient) DiscoverResources(
	ctx context.Context,
	gvk schema.GroupVersionKind,
