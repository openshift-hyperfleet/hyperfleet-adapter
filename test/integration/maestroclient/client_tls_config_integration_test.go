package maestroclientintegration
...
const (
	testInsecureTrue = "true"
)

// buildMaestroClientConfigFromLoaded reproduces the same mapping logic as createMaestroClient
// in cmd/adapter/main.go. This is intentionally duplicated so the test catches drift between
// main.go and the config structs.
func buildMaestroClientConfigFromLoaded(
	...
	if maestroConfig.Timeout != "" {
		...
		if err != nil {
			return nil, fmt.Errorf(
				"invalid maestro timeout %q: %w", maestroConfig.Timeout, err,
			)
		}
	...
	if maestroConfig.ServerHealthinessTimeout != "" {
		...
		if err != nil {
			return nil, fmt.Errorf(
				...
				maestroConfig.ServerHealthinessTimeout, err,
			)
		}
...
// writeTestAdapterConfig writes a minimal AdapterConfig YAML that references the given TLS
// cert paths and Maestro addresses.
func writeTestAdapterConfig(t *testing.T, dir string, opts map[string]string) string {
	...
	insecure := "false"
	if opts["insecure"] == testInsecureTrue {
		insecure = testInsecureTrue
	}
...
// TestTLSConfigLoadAndConnect_MutualTLS loads an AdapterConfig YAML with mTLS settings,
// parses it through the config loader, maps it to maestroclient.Config (same logic as
// main.go), creates a real client, and connects to the TLS Maestro.
func TestTLSConfigLoadAndConnect_MutualTLS(t *testing.T) {
	...
	require.NoError(
		t, err, "ListManifestWorks over TLS (config-loaded mTLS) should succeed",
	)
	t.Logf("Config-loaded mTLS: listed %d ManifestWorks", len(list.Items))
}

// TestTLSConfigLoadAndConnect_CAOnly loads config with only caFile (no client certs), verifying
// the CA-only TLS path works end-to-end from config file.
func TestTLSConfigLoadAndConnect_CAOnly(t *testing.T) {
	...
	list, err := tc.Client.ListManifestWorks(tc.Ctx, "test-cluster-list", "")
	require.NoError(
		t, err, "ListManifestWorks over TLS (config-loaded CA-only) should succeed",
	)
	t.Logf("Config-loaded CA-only: listed %d ManifestWorks", len(list.Items))
}

// TestTLSConfigLoadAndConnect_Insecure loads an insecure config (no TLS) and connects to the
// plaintext Maestro, verifying the insecure path from config.
...
// TestTLSConfigLoadAndConnect_EnvOverride verifies that environment variables override YAML
// config values, simulating the production Viper override path.
func TestTLSConfigLoadAndConnect_EnvOverride(t *testing.T) {
	...
	list, err := tc.Client.ListManifestWorks(tc.Ctx, "test-cluster-list", "")
	require.NoError(
		t, err, "ListManifestWorks with env-overridden TLS config should succeed",
	)
