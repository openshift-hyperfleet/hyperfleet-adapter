// main_test.go provides shared test setup for Maestro integration tests. It starts PostgreSQL
// and Maestro server containers that are reused across all test functions.
...
func TestMain(m *testing.M) {
	...
	// Skip on ARM64 Macs unless MAESTRO_ARM64_TEST is set (user has local ARM64 image). To run
	// on ARM64, build a local image from the Maestro source and tag it as:
	...
	if runtime.GOARCH == "arm64" && os.Getenv("MAESTRO_ARM64_TEST") != "true" {
		skipReason = "ARM64 architecture without MAESTRO_ARM64_TEST=true " +
			"(set this env if you have a local ARM64 Maestro image)"
		...
		println(
			"   cd /path/to/maestro && " +
				"podman build -t quay.io/redhat-user-workloads/maestro-rhtap-tenant/maestro/maestro:latest .",
		)
		os.Exit(m.Run())
	}
