...
// to resolving the path relative to the project root using runtime.Caller.
func getConfigPath() string {
	...
	return filepath.Join(projectRoot, "configs/adapter-config-template.yaml")
}
...
func TestConfigLoadAndCriteriaEvaluation(t *testing.T) {
	...
	config, err := config_loader.Load(configPath, config_loader.WithBaseDir(filepath.Dir(configPath)))
